...
export default class MatchModal extends React.Component {
    ...
    editResult = (editableResult) => {
        ...
        if (editableResult){
            ...
            matchPlayers = [this.matchPlayers[0][1],this.matchPlayers[1][1]]
            ...
            if (this.resultIsCorrect(editedResults)){
                ...
                this.props.navigation.navigate("Clasifications")
            } else {
    ...
    renderAddResultButton = (addResultButton) => {
        ...
        if ((addResultButton && !this.state.resultSubmitted && matchPlayersNames.indexOf(this.playerName) >= 0) || ["Pol Febrer","Dani Ramírez"].indexOf(this.playerName) >= 0) {
            ...
            button = (
                <View style={{flexDirection:"row",marginBottom: 20}}>
                    <TouchableOpacity style={styles.goBackArrowButton} onPress={()=>{this.props.navigation.navigate("Clasifications")}}>
            ...
            button = (
                <TouchableOpacity key="goBackButton" style={styles.goBackButton} onPress={()=>{this.props.navigation.navigate("Clasifications")}}>
  ...
  render() {
    ...
    const sets = fromChallenges ? points : points.map((point) => {return this.pointsToSets(point)})
    ...
    <ImageBackground style = {{flex:1}} source={require("../assets/bg.jpg")}>
      <View style= {styles.container}> 
        <View style={styles.scoreContainer}>
            ...
            <ScoreRow sets={this.state.resultSubmitted? this.editedResults : sets} onNewResult={this.getEditedResults} editableResult={this.state.editableResult}/> 
        </View>
        <View style={styles.matchPlayersView}>

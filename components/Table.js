...
export default class Table extends React.Component {
  getTotals = (myArray, chunk_size) => {
    ...
    while (myNewArray.length) {
      totals.push(myNewArray.splice(0, chunk_size).reduce((a,b) => {return a+b;}));
    }
    return totals;
  }
  ...
  renderTable = (group,iGroup,groupResults) => {
    ...
    //Construïm taula
      let table = []
    //Títol del grup
      table.push (<View key={String(iGroup)+"header"} style={styles.tableTitle}><Text style={styles.tableTitleText}>GRUP {iGroup}</Text></View>)
    //Taula en sí
      for (let i = 0; i <= nGroup; i++){
          ...
          for (let j = 0; j < nGroup+3; j++){
              ...
              if (typeOfCell == "playerNameCell" || typeOfCell == "pointsCell"){
                rowContent.push(
                  <TouchableOpacity key={String(iGroup)+String(i)+String(j)} style={[styles.tableCell,addStyles]} onPress={() => {this.props.handlePress(toSendOnPress)}}>
            ...
            if (i == 0){addRowStyles = styles.headerRow}
